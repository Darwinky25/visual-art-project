...
let c = fs.readFileSync('src/visualizer/drawMedia.ts', 'utf8');

c = c.replace(
  /export function drawMedia\([\s\S]*?ctx\.drawImage\(element, dx, dy, dw, dh\);\n\}/,
...
fs.writeFileSync('src/visualizer/drawMedia.ts', c);
